...
llm_chain_wenti = LLMChain(prompt=prompt_wenti, llm=agent)
llm_chain_jianli = LLMChain(prompt=prompt_jianli, llm=agent)

def get_bot_answer(question):
    """获取聊天机器人回答"""
    return llm_chain.run({"q": question})

def get_bot_answer_wenti(wenti):
    """获取聊天机器人回答,截图"""
    return llm_chain_wenti.run({"wenti": wenti})
def get_bot_answer_jianli(jianli):
    """获取聊天机器人回答，制造简历"""
    return llm_chain_jianli.run({"jianli": jianli})
